...
/* Example finished version (don't spoil the exercise!) */
...
const alwaysSunny = buildTemperatureMock(75, 'F')

/** Note: don't try this outside of an async function.*/
async function demo() {
    // weatherReportProvidence can't tell the difference.
    console.log(`real data (beware: this might be the same as the mock :-)):`)
