...
public class HoursDispatcher {
    ...
    public void dispatch()  {
        System.out.println("Dispatcher: Welcome to TA hours! Today we're discussing "+this.getStatusMessage());
        while(true) {
            if(queue.hasNext()) {
                for(TA ta : this.minutesLeft.keySet()) {
                    if(ta.isFree()) {
                        Student nextStudent = queue.next();
                        System.out.println("Dispatcher: Hi "+nextStudent+"; you'll be seen by "+ta);
                ...
                try {
                    ...
                    System.out.println("Dispatcher: Nobody waiting in queue, will check again in three seconds.");
                    Thread.sleep(3000);
